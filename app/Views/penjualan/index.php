    ...
    <div class="row">
            ...
            <div class="card card-primary card-outline">
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="user" id="user" readonly value="<?= aduh('212412_nama') ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="pelanggan" class="col-sm-3 col-form-label">Pelanggan</label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="pelanggan" id="pelanggan" readonly value="Umum">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="card card-primary card-outline">
                <div class="card-body">
                    <div class="form-group row">
                        <label for="barcode" class="col-sm-3 col-form-label">No.Produk</label>
                        <div class="col-sm-9">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control mr-2" id="barcode" name="barcode" placeholder="Cari No.Produk / nama barang" autofocus autocomplete="off">
                    ...
                    <table class="table table-bordered table-striped" id="tabel-keranjang" width="100%">
                        <thead>
                            <tr>
                                ...
                                <th>Harga</th>
                                <th>Jumlah</th>
                                <th style="width: 150px;">Diskon Prod (%)</th>
                                <th>Total</th>
                                <th>Aksi</th>
                            </tr>
                        </thead>
...
<script>
    $(document).ready(function() {
        ...
        function detailKeranjang() {
            let keranjang = ''
            $.ajax({
                url: "<?= site_url(); ?>/Penjualan/keranjang",
                dataType: 'json',
        ...
        // pertama halaman dibuka load detail keranjang
        detailKeranjang()

        // Cari item berdasarkan barcode
        $('#barcode').autocomplete({
            source: "<?= site_url(); ?>/produk/barcode",
            autoFocus: true,
        ...
        function kalkulasi() {
            let sub_total = $('#sub_total').val(),
                ...
                tunai = $('#tunai').val().replace(/\./g, ''), // Menghapus semua titik dari nilai tunai
        ...
        $('.wrapper').on('click', '#bayar', function() {
            let namapelanggan = $('#pelanggan').val()
                            ...
                            success: function(response) {
                                if (response.status) {
                                    Swal.fire({
                                        ...
                                        window.open(
                                            `<?= site_url(); ?>/penjualan/cetak/${response.idpenjualan}`
                                        )
